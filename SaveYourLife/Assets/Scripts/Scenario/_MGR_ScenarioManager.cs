...
public class _MGR_ScenarioManager : MonoBehaviour
{
    ...
    public List<Scenario_Etape> p_etapes ;         // pour stocker les étapes & private
    ...
    public void Configurer( Scenario_Etape[] __etapes_du_scenario)
    {
        ...
        if (p_etapes != null) p_etapes.Clear();         // vider le tableau (si 2ème ou suivantes exécutions....
        else p_etapes = new List<Scenario_Etape>();     // 1ere exécution : instantiation
        nbEtapes = 0;
        foreach (Scenario_Etape _etape in __etapes_du_scenario)
        {
            // un objet valide ?
            if (_etape.interactiveObject == null) CommonDevTools.QUIT_APP("objet étape " + nbEtapes + " non défini");
            // un type d'intearction défini ? 
            if (_etape.interaction == Scenario_Etape.Interactions.UNDEFINED) CommonDevTools.ERROR("interaction étape " + nbEtapes + " non définie", this.gameObject);
